            ...
            for (var browserId in hasData.results) {
                ...
                for (var i = 0; i < featureTests.length; ++i) {
                    ...
                    if (feature in allTests) {
                        var has = allTests[feature].result == '1' ? true : false;
        ...
        async.map(fileNames,
            ...
            abind(function(err, detects) {
                ...
                fs.readFile(hasjsPath, abind(function(err, source) {
                    ...
                    if (minify) {
                        var ast = transformjs.transform(js);
                        js = transformjs.generate(ast, true);
                    }
                    cb(0, js);
                }, cb, this))
            }, cb, this)
 ...
 */
exports.elminateFeatureTests = function(source, featureMap, minify, beautify) {
